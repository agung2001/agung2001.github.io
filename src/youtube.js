export default [
	{
		name: '🧑‍💻CERMAT',
		about: 'This playlist contains tutorials, tips, and tricks about technology in general.',
		url: 'https://www.youtube.com/watch?v=5T19hs4S8Z8&list=PLc4H2My9zgNV7agwL72OCsl0cidduls0O',
		videos: [
			{
				name: 'Obsidian Note : Aplikasi pencatat buat orang pinter?',
				url: 'https://www.youtube.com/watch?v=r4uv11poc0k',
			},
			{
				name: 'Cara Optimal Mengatur Waktu Kerja dan Belajar dengan Aplikasi Time Tracking Clockify',
				url: 'https://youtu.be/AIi9YP3gBjs',
			},
			{
				name: 'KeePassXC Aplikasi Password Manager Gratis yang wajib Kamu Install Kalo Kamu Gak Mau ke',
				url: 'https://youtu.be/523eSxVhtWk',
			},
			{
				name: 'Cara Belajar dan Bekerja 2x Lebih Cepat dengan Playback Speed Technique',
				url: 'https://youtu.be/rRxpscQi67Q',
			},
			{
				name: 'Cara Mudah Mengatur Volume di Browser dengan Ekstensi Volume Booster',
				url: 'https://youtu.be/iA9nD5zz030',
			},
			{
				name: 'Digital Minimalism: Cara Tetap Produktif Membebaskan Diri dari Ketergantungan Gadget & Media Sosial',
				url: 'https://youtu.be/jz_yCVxBICU',
			},
			{
				name: 'Revolusi Membaca di Era Digital : Ekspresi Literasi di Era Teknologi',
				url: 'https://youtu.be/GNist7NEZ9E',
			},
			{
				name: 'Generative AI : Mantra Kemajuan atau Kutukan Karir dan Profesi Manusia',
				url: 'https://youtu.be/M2CsSCYCq9c',
			},
			{
				name: 'Stable Diffusion: Teknologi AI Gratis untuk Menciptakan Gambar Menakjubkan',
				url: 'https://www.youtube.com/watch?v=I4OLR94CHCo',
			},
			{
				name: 'Prompt Engineering: Seni Berkomunikasi dengan AI Secara Efektif',
				url: 'https://youtu.be/B-5vC523llM',
			},
		],
	},
	{
		name: '🤖 KATA',
		about: 'This playlist contains tutorials, tips, and tricks about programming.',
		url: 'https://www.youtube.com/watch?v=oe5a-2OVUco&list=PLc4H2My9zgNVWe2apGHnUmWpsAKBaAvVo',
		videos: [
			{
				name: 'ASPRI Asisten Pribadi Koleksi Script-Library untuk meningkatkan produktifitas developer',
				url: 'https://youtu.be/oe5a-2OVUco',
			},
			{
				name: 'Panduan Praktis Memahami dan Mengkonfigurasi Protokol SSH untuk Pemula dan Expert',
				url: 'https://youtu.be/YeQSwp_AELg',
			},
			{
				name: 'Udah Punya Rsync untuk File Transfer - Masih Juga Cari yang Lain?',
				url: 'https://youtu.be/SgctZG6OEnw',
			},
			{
				name: 'Membedah Konsep Skema Software Versioning untuk Pemula dan Expert',
				url: 'https://youtu.be/7mtt9OYuH_M',
			},
			{
				name: 'Cara menulis Commit Message yang baik dengan Conventional Commits',
				url: 'https://youtu.be/TudmCr2OE2E',
			},
		],
	},
	{
		name: '🤖 KATA INFRA',
		about: 'This playlist contains tutorials, tips, and tricks about infrastructure.',
		url: 'https://www.youtube.com/watch?v=OmPf5SkxW98&list=PLc4H2My9zgNWb3TEw28o-MFP04ryjisLk',
		videos: [
			{
				name: 'Cara Membangun Cloud Storage Alternative Seperti Google Drive untuk BISNIS dan KORPORASI dengan Syncthing!',
				url: 'https://www.youtube.com/watch?v=OmPf5SkxW98',
			},
			{
				name: 'Incremental Backup dengan Duplicati - Simpan Data Pribadi dan Perusahaan di Manapun dengan Aman',
				url: 'https://youtu.be/piQMaPDcKjQ',
			},
		],
	},
	{
		name: '🤖 KATA WordPress',
		about: 'This playlist contains tutorials, tips, and tricks about Programming in WordPress.',
		url: 'https://www.youtube.com/watch?v=VImZA0DjUbk&list=PLc4H2My9zgNXpnhG1JO5eM-WmZ2Sa4O95',
		videos: [
			{
				name: 'Cara Install WordPress Local Untuk Pemula dengan Local by Flywheel',
				url: 'https://youtu.be/VImZA0DjUbk',
			},
			{
				name: 'Cara Cepat Memahami WordPress dengan Belajar Struktur File & Directory',
				url: 'https://www.youtube.com/watch?v=fMiKALXOxC4',
			},
			{
				name: 'Cara Cepat Memahami WordPress dengan Belajar Skema Database',
				url: 'https://www.youtube.com/watch?v=ANaR0TA0jO0',
			},
			{
				name: 'Cara Cepat Menjadi Developer WordPress dengan Belajar Hook',
				url: 'https://www.youtube.com/watch?v=NfT7LvfStbI',
			},
			{
				name: '10 WordPress Hooks Paling Populer yang Developer Wajib Tau',
				url: 'https://www.youtube.com/watch?v=9UKf_sDWx6g',
			},
			{
				name: 'Cara Bongkar Pasang Content WordPress dengan Shortcode',
				url: 'https://www.youtube.com/watch?v=QyXq8WTo2bM',
			},
		],
	},
	{
		name: '🤖 KATA WordPress Plugin',
		about: 'This playlist contains tutorials, tips, and tricks about Programming a WordPress Plugin.',
		url: 'https://www.youtube.com/watch?v=xbTiYXXkF-w&list=PLc4H2My9zgNWAVFQiK3nRpnSOLByUh8Wn',
		videos: [
			{
				name: 'Cara Membuat WordPress Popup:Modal Plugin Sederhana',
				url: 'https://youtu.be/xbTiYXXkF-w',
			},
			{
				name: 'Dot Framework kerangka mutakhir untuk pembuatan WordPress Plugin dan Theme',
				url: 'https://youtu.be/kYyBj3_uICE',
			},
		],
	},
	{
		name: '🤖 KATA WooCommerce',
		about: 'This playlist contains tutorials, tips, and tricks about Programming in WooCommerce.',
		url: 'https://www.youtube.com/watch?v=FE19MuS2-Rg&list=PLc4H2My9zgNV8zIq-rXkSmQcjBTIoMXsh',
		videos: [
			{
				name: 'WooCommerce 101 : Memahami Platform E-Commerce Terpopler di Dunia!',
				url: 'https://youtu.be/FE19MuS2-Rg',
			},
		],
	},
	{
		name: '🧳 PAHAM Manajemen',
		about: 'This playlist contains tutorials, tips, and tricks about management in general.',
		url: 'https://www.youtube.com/watch?v=_1Dq1kQn29E&list=PLc4H2My9zgNVfHx-wV_vgGCMsDN1nkzuU',
		videos: [
			{
				name: 'Cara Mudah Membuat Standard Operating Procedure (SOP) dengan 3S/BDA',
				url: 'https://youtu.be/_1Dq1kQn29E',
			},
			{
				name: 'Template Balance Scorecard (BSC) Sederhana untuk Mengukur Kinerja UMKM & Korporasi',
				url: 'https://youtu.be/9nuZKOxAKJg',
			},
			{
				name: 'Cara membangun budaya Kolaborasi & Inovasi di Perusahaan dengan bantuan Otak Kedua',
				url: 'https://youtu.be/lbKr3ylzCjE',
			},
			{
				name: 'Cara membangun budaya Disiplin & Kegigihan di Perusahaan dengan Chourei (ちょうれい)  Morning Assembly',
				url: 'https://youtu.be/rq5SLdgXTZA',
			},
			{
				name: 'YouTrack untuk Pemula: Optimalkan Alur Kerja Tim dengan Modern Project Management Tool',
				url: 'https://youtu.be/JeV2j7CUsFw',
			},
			{
				name: 'Cara Mengidentifikasi dan Memprioritaskan Project/Rock yang Tepat untuk Mendorong Pertumbuhan Bisnis',
				url: 'https://youtu.be/TOa0GtrvJN0',
			},
			{
				name: 'Membangun Mesin Waktu: Perjalanan Menuju Produktivitas Tanpa Batas dengan Komunikasi Asynchronous',
				url: 'https://youtu.be/ZV04iaFbl_s',
			},
			{
				name: 'Demokratisasi Talenta: Peran Pekerja Remote dalam Pemerataan Ekonomi Indonesia',
				url: 'https://youtu.be/wX5HJUNHrF8',
			},
			{
				name: 'Vision Traction Organizer (VTO) : Cara merangkai Visi menjadi Aksi dalam bisnis',
				url: 'https://youtu.be/S0PRrg4roS8',
			},
			{
				name: 'Memaksimalkan Produktifitas : Seni Mengelola Tugas Kerja bagi Team & Individu',
				url: 'https://youtu.be/jz4OSrNZ2Dk',
			},
		],
	},
	{
		name: '🧳 PAHAM Karir',
		about: 'This playlist contains tutorials, tips, and tricks about career in general.',
		url: 'https://www.youtube.com/watch?v=9QB2tvtLmfo&list=PLc4H2My9zgNUgsn4LO2cP5wvSc5d7_UcM',
		videos: [
			{
				name: 'Lebih dari 10000 Jam - Mengungkap Konsep Kesuksesan dan Strategi Menggapainya Outliers',
				url: 'https://youtu.be/9QB2tvtLmfo',
			},
			{
				name: 'Growth Mindset: Strategi Psikologis Menghadapi Perubahan dan Menggapai Potensi Penuh Individu',
				url: 'https://youtu.be/Q7_Hnt11gx0',
			},
		],
	},
	{
		name: '💰 TUNAI',
		about: 'This playlist contains tutorials, tips, and tricks about finance in general.',
		url: 'https://www.youtube.com/watch?v=5T19hs4S8Z8&list=PLc4H2My9zgNV7agwL72OCsl0cidduls0O',
		videos: [
			{
				name: 'Membuat Personal Spending Tracker (PST) Untuk Pencatatan Keuangan Pribadi',
				url: 'https://youtu.be/5T19hs4S8Z8',
			},
			{
				name: 'Cara Mudah Mengelola Gaji dan Jam Kerja dengan Personal Income Tracker (PIT)',
				url: 'https://youtu.be/x3gfe3jNt_A',
			},
			{
				name: 'Cara Mudah Mengelola Subscription Service dengan Subscription Management Template (SMT)',
				url: 'https://youtu.be/Y7o3IPdc1oo',
			},
			{
				name: 'Strategi Membangun Harta Kekayaan: Panduan Investasi dan Pengembangan Portfolio Asset',
				url: 'https://youtu.be/cfb18CHrLnY',
			},
			{
				name: 'Cara Mudah Mengelola Personal Asset dengan Personal Asset Management Tracker (PAMT)',
				url: 'https://www.youtube.com/watch?v=-IK1BElY074',
			},
			{
				name: 'Seni Mengelola Anggaran: Panduan Mengelola Keuangan Pribadi dengan Bijak',
				url: 'https://youtu.be/Eq6yPlzTt0c',
			},
		],
	},
	{
		name: '🗣️ BICARA',
		about: 'This playlist contains my talks as speaker or guest.',
		url: 'https://www.youtube.com/watch?v=5T19hs4S8Z8&list=PLc4H2My9zgNV7agwL72OCsl0cidduls0O',
		videos: [
			{
				name: 'Utilization of Digital Tools To Increase Productivity and Professionalism | JSC Lab Sharing #13',
				url: 'https://youtu.be/UlLyR23_5hg',
			},
		],
	},
]
